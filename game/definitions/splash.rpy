...
image menu_particles:
    ...
    xpos 224
    ypos 104
    ParticleBurst("mod_assets/menu/menu_particle.png", explodeTime=0, numParticles=40, particleTime=2.0, particleXSpeed=3, particleYSpeed=3).sm
    particle_fadeout
...
label splashscreen:
    ...
    if renpy.version_tuple == (6, 99, 12, 4, 2187) and not renpy.get_autoreload():
        if os.path.exists(config.gamedir + "/definitions/splash.rpy"):
            "{b}Uyarı:{/b} DDLC Mod Şablonunu, yakın gelecekte amortismana tabi tutulabilecek bir Ren'Py sürümünde çalıştırıyorsunuz."
            "Mod Şablonu geliştirme, Ren'Py 7 ve Ren'Py 8'de DDLC'yi desteklemeye odaklanmıştır."
            "Bu şablon mevcut Ren'Py sürümünü desteklese de yakın gelecekte durum böyle olmayabilir."
            "Mod geliştirmeye devam etmek için Ren'Py 7'ye yükseltmeniz önemle tavsiye edilir. Daha fazla bilgiyi [burada](https://www.reddit.com/r/DDLCMods/wiki/notices/#wiki_why_is_the_megathread_and_other_users_recommending_me_to_create_my_mod_in_ren.27py_7.3F) bulabilirsiniz."
            window hide
            pause 1.0
            window auto
    ...
    if not persistent.first_run:
        ...
        if extra_settings:
            if process_check(["obs32.exe", "obs64.exe", "obs.exe", "xsplit.core.exe", "livehime.exe", "pandatool.exe", "yymixer.exe", "douyutool.exe", "huomaotool.exe"]):
                ...
                call screen dialog("Haydi Oynayalım Modu otomatik olarak etkinleştirildi.\nBu mod, hassas bilgiler içeren içeriği\natlamanıza veya alternatif\nhikaye seçenekleri uygulamanıza olanak tanır.\n\nBu ayar, mod geliştiricinin bu kontrolleri hikayesinde programlayıp programlamadığına\nbağlı olacaktır.\ n\n Hadi Oynayalım Modunu kapatmak için Ayarlar'ı ziyaret edin ve\nHadi Oynayalım Modu'nu işaretleyin.", 
                    [Hide("dialog"), Return()])
        scene white
...
label after_load:
    ...
    if anticheat != persistent.anticheat:
        ...
        if persistent.playername == "":
            ...
            m "Çok komiksin, [persistent.playername]."
        $ renpy.utter_restart()
    else:
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            $ persistent.first_load = True
            call screen dialog("İpucu: Okuduğunuz metinde\nhızlı ileri sarmak için \"Atla\" düğmesini kullanabilirsiniz.", ok_action=Return())
    return
